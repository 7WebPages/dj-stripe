...
class Customer(StripeObject):
    ...
    valid_payment_method = models.BooleanField(default=False, verbose_name='Valid payment method')
    ...
    def set_account_balance(self, amount):
        """
        Set credit for user account balance
        """
        customer = stripe.Customer.retrieve(self.stripe_id)
        current_balance = customer.account_balance / 100
        customer.account_balance = (decimal.Decimal(-int(amount)) + current_balance) * 100
        customer.save()

    def calculate_debt(self):
        """
        calculates the number of days payable
        :return: 0
        """
        delta = (self.current_subscription.current_period_end - timezone.now()).days
        sum_of_days = (self.current_subscription.current_period_end - self.current_subscription.current_period_start).days

        sum_amount = (delta * self.current_subscription.amount) / sum_of_days
        if delta < 0:
            return 0
        return sum_amount
    ...
    def set_default_card(self, token):
    ...
    def subscribe_without_card(self, plan, card):
        """
            If user make payments from standart stripe form
        """
        cu = self.stripe_customer

        resp = cu.subscriptions.create(
            plan=Plan.objects.get(stripe_id=plan).stripe_id,
            card=card
        )
    ...
    def one_time_pay(self, card, amount, customer, currency="usd"):
        """
            Charge - one time payment
        """
        # Link card to the account
        # TODO: maybe need remove
        card = customer.stripe_customer.cards.create(card=card)
        ...
        Charge.sync_from_stripe_data(res)
...
class CurrentSubscription(TimeStampedModel):
    ...
    def refund(self):
        ...
        duration_days = self.current_period_end.date().days - self.current_period_start.date().days
        spent_days = datetime.date.today().days - self.current_period_start.date().days
